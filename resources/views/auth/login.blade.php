...
@section('title', 'Masuk — Loose Tape')
    ...
    <div style="width:100%;max-width:420px;position:relative;z-index:1;">
        ...
        <div class="sticker-hover" style="position:absolute;bottom:-40px;right:-46px;width:96px;transform:rotate(-8deg);z-index:4;--r:-8deg;">
            <div style="background:#161514;border:2px solid #F1EFE8;padding:10px 8px;clip-path:polygon(0 6%,100% 0,100% 94%,0 100%);box-shadow:4px 5px 0 rgba(0,0,0,0.35);">
                <div style="font-family:'Archivo Black',sans-serif;font-size:15px;color:#FF1F8F;line-height:1;text-align:center;">SIDE<br>B</div>
            </div>
        </div>
        ...
        <div class="stamp-label">AKSES SISTEM</div>

        <div class="torn-card">
            <div class="torn-card-shadow"></div>
            <div class="torn-card-body">
                <div class="headline-stamp" style="margin:12px 0 8px;">
                    <span class="shadow-text" style="font-size:30px;letter-spacing:-1px;top:3px;left:4px;opacity:1;">MASUK</span>
                    <span class="main-text" style="font-size:30px;letter-spacing:-1px;color:#161514;transform:rotate(-1.5deg);">MASUK</span>
                </div>
                <div style="font-size:13px;color:#5F5E5A;margin-bottom:28px;">Login untuk mengakses sistem reservasi studio.</div>

                @if ($errors->any())
                    <div class="alert-error">{{ $errors->first() }}</div>
                @endif

                <form method="POST" action="{{ route('login.attempt') }}" style="display:flex;flex-direction:column;gap:16px;">
                    @csrf
                    <label style="display:flex;flex-direction:column;gap:6px;">
                        <span style="font-family:'Archivo Black',sans-serif;font-size:11px;letter-spacing:0.03em;color:#5F5E5A;">EMAIL</span>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="CTH. ADMIN@LOOSETAPE.TEST" required
                    ...
                    <button type="submit" class="btn-pink" style="margin-top:8px;font-size:14px;padding:12px;width:100%;">MASUK &rarr;</button>
                    <div style="text-align:center;font-size:12px;color:#5F5E5A;">Belum punya akun? <a href="{{ route('register') }}" style="color:#FF1F8F;text-decoration:underline;font-weight:bold;">Daftar di sini</a></div>
                </form>
            </div>
        </div>
    </div>
